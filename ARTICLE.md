...
The second was a review of my options-backtesting engine, the one that has to sign off on a defined-risk double calendar on SPX before a nickel of real capital goes near it. This isn't curve-fit-and-pray. Every candidate configuration is chosen by nested walk-forward analysis: the parameters are searched only on in-sample training folds, then judged on out-of-sample folds the search was never allowed to see, so the number I actually trust is the one the optimizer never got to touch. The engine models the intraday adjustment the strategy makes when the spread's mark crosses a first-passage threshold, prices fills off the NBBO midpoint instead of last trades, masks quotes that have gone stale past a cap, applies a fill haircut, and then reports P&L twice, once on real market marks and once on the trades it had to model, so I can see exactly how much of the "edge" is real versus assumed.
